...
struct kvm_shadow_walk_iterator {
  ...
  int start_level;
	unsigned index;
  bool obsolete;
  struct kvm_mmu_page *pages[PT64_ROOT_MAX_LEVEL];
};

#define for_each_level(_level, _lo, _hi) \
  for(_level = (_hi); _level >= (_lo); _level--)
...
#define for_each_shadow_entry_locked(_vcpu, _addr, _walker)            \
	for (shadow_walk_init(&(_walker), _vcpu, _addr);	\
	     shadow_walk_okay_locked(&(_walker));			\
	     shadow_walk_next(&(_walker)))
...
static inline bool sp_read_lock(struct kvm *kvm, struct kvm_mmu_page *sp)
{
  read_lock(&sp->lock);
  return is_obsolete_sp(kvm, sp);
}

static inline int sp_try_read_lock(struct kvm *kvm, struct kvm_mmu_page *sp)
{
  if (!read_trylock(&sp->lock))
    return -EBUSY;

  return is_obsolete_sp(kvm, sp);
}

static inline void sp_read_unlock(struct kvm *kvm, struct kvm_mmu_page *sp)
{
  read_unlock(&sp->lock);
}

static inline bool sp_upgrade_lock(struct kvm *kvm, struct kvm_mmu_page *sp)
{
  read_unlock(&sp->lock);
  write_lock(&sp->lock);
  return is_obsolete_sp(kvm, sp);
...
static inline bool sp_write_lock(struct kvm *kvm, struct kvm_mmu_page *sp)
{
  write_lock(&sp->lock);
  return is_obsolete_sp(kvm, sp);
}

static inline int sp_try_write_lock(struct kvm *kvm, struct kvm_mmu_page *sp)
{
  if (!write_trylock(&sp->lock))
    return -EBUSY;

  return is_obsolete_sp(kvm, sp);
}

static inline void sp_write_unlock(struct kvm *kvm, struct kvm_mmu_page *sp)
{
  write_unlock(&sp->lock);
}

static inline bool sp_downgrade_lock(struct kvm *kvm, struct kvm_mmu_page *sp)
{
  write_unlock(&sp->lock);
  read_lock(&sp->lock);
  return is_obsolete_sp(kvm, sp);
...
static int mmu_topup_memory_caches(struct kvm_vcpu *vcpu, bool maybe_indirect)
{
	...
	r = kvm_mmu_topup_memory_cache(&vcpu->arch.mmu_pte_list_desc_cache,
			KVM_ARCH_NR_OBJS_PER_MEMORY_CACHE_MIN);
	if (r)
		return r;
					    ...
					    gfn_t gfn, int count)
{
	...
	for (i = KVM_MAX_HUGEPAGE_LEVEL; i >= PG_LEVEL_2M; i--) {
		linfo = lpage_info_slot(gfn, slot, i);
    write_lock(&linfo->lock);
		linfo->disallow_lpage += count;
    write_unlock(&linfo->lock);
...
void untrack_possible_nx_huge_page(struct kvm *kvm, struct kvm_mmu_page *sp)
{
  ...
  sp_write_lock(kvm, sp);
	list_del_rcu(&sp->possible_nx_huge_page_link);
...
static bool invalid_list_empty(struct kvm_mmu_invalid_list *invalid_list)
{
  int level;
  for(level = 0; level <= PT64_ROOT_MAX_LEVEL; level++)
    if(!list_empty(&invalid_list->heads[level]))
      return false;
  return true;
}

#define INIT_INVALID_LIST(invalid_list) \
  do { \
    int level; \
    for(level = 0; level <= PT64_ROOT_MAX_LEVEL; level++) \
      INIT_LIST_HEAD(&invalid_list.heads[level]); \
  } while(0)
...
static void ___kvm_rmap_read_begin(struct kvm_rmap_head *rmap_head)
{
  read_lock(&rmap_head->lock);
}

static void __kvm_rmap_read_begin(struct kvm_memory_slot *slot, gfn_t gfn)
{
  ...
  ___kvm_rmap_read_begin(rmap_head);
}

void kvm_rmap_read_begin(struct kvm *kvm, gfn_t gfn)
{
  __kvm_rmap_read_begin(gfn_to_memslot(kvm, gfn), gfn);
}

void kvm_vcpu_rmap_read_begin(struct kvm_vcpu *vcpu, gfn_t gfn)
{
  kvm_rmap_read_begin(vcpu->kvm, gfn);
}


static void ___kvm_rmap_write_begin(struct kvm_rmap_head *rmap_head)
{
  write_lock(&rmap_head->lock);
}

static void __kvm_rmap_write_begin(struct kvm_memory_slot *slot, gfn_t gfn)
{
  struct kvm_rmap_head *rmap_head;
  unsigned long idx;
  int level;
  ...
  idx = gfn_to_index(gfn, slot->base_gfn, level);
  rmap_head = &slot->arch.rmap[level - PG_LEVEL_4K][idx];
  ___kvm_rmap_write_begin(rmap_head);
}

void kvm_rmap_write_begin(struct kvm *kvm, gfn_t gfn)
{
  __kvm_rmap_write_begin(gfn_to_memslot(kvm, gfn), gfn);
}

void kvm_vcpu_rmap_write_begin(struct kvm_vcpu *vcpu, gfn_t gfn)
{
  kvm_rmap_write_begin(vcpu->kvm, gfn);
}


void ___kvm_rmap_read_end(struct kvm_rmap_head *rmap_head)
{
  read_unlock(&rmap_head->lock);
}


void __kvm_rmap_read_end(struct kvm_memory_slot *slot, gfn_t gfn)
{
  struct kvm_rmap_head *rmap_head;
  unsigned long idx;
  int level;
  ...
  idx = gfn_to_index(gfn, slot->base_gfn, level);
  rmap_head = &slot->arch.rmap[level - PG_LEVEL_4K][idx];
  ___kvm_rmap_read_end(rmap_head);
}

void kvm_rmap_read_end(struct kvm *kvm, gfn_t gfn)
{
  __kvm_rmap_read_end(gfn_to_memslot(kvm, gfn), gfn);
}

void kvm_vcpu_rmap_read_end(struct kvm_vcpu *vcpu, gfn_t gfn)
{
  kvm_rmap_read_end(vcpu->kvm, gfn);
}

void ___kvm_rmap_write_end(struct kvm_rmap_head *rmap_head)
{
  write_unlock(&rmap_head->lock);
}

void __kvm_rmap_write_end(struct kvm_memory_slot *slot, gfn_t gfn)
{
  struct kvm_rmap_head *rmap_head;
  unsigned long idx;
  int level;
  ...
  idx = gfn_to_index(gfn, slot->base_gfn, level);
  rmap_head = &slot->arch.rmap[level - PG_LEVEL_4K][idx];
  ___kvm_rmap_write_end(rmap_head);
}

void kvm_rmap_write_end(struct kvm *kvm, gfn_t gfn)
{
  __kvm_rmap_write_end(gfn_to_memslot(kvm, gfn), gfn);
}

void kvm_vcpu_rmap_write_end(struct kvm_vcpu *vcpu, gfn_t gfn)
{
  kvm_rmap_write_end(vcpu->kvm, gfn);
}
...
static void pte_list_desc_remove_entry_vcpu(struct kvm_vcpu *vcpu,
				       struct kvm_rmap_head *rmap_head,
				       struct pte_list_desc *desc, int i)
{
	struct pte_list_desc *head_desc = (struct pte_list_desc *)(rmap_head->val & ~1ul);
	 ...
	 * removed (this flow) when they become empty.
	 */
	KVM_BUG_ON_DATA_CORRUPTION(j < 0, vcpu->kvm);
	...
	if (!head_desc->more)
		...
		rmap_head->val = (unsigned long)head_desc->more | 1;
	kvm_mmu_memory_cache_free(&vcpu->arch.mmu_pte_list_desc_cache, head_desc);
}
...
static void pte_list_remove_vcpu(struct kvm_vcpu *vcpu, u64 *spte,
			    struct kvm_rmap_head *rmap_head)
{
	struct pte_list_desc *desc;
	int i;

	if (KVM_BUG_ON_DATA_CORRUPTION(!rmap_head->val, vcpu->kvm))
		return;

	if (!(rmap_head->val & 1)) {
		if (KVM_BUG_ON_DATA_CORRUPTION((u64 *)rmap_head->val != spte, vcpu->kvm))
			return;
		...
		while (desc) {
			for (i = 0; i < desc->spte_count; ++i) {
				if (desc->sptes[i] == spte) {
					pte_list_desc_remove_entry_vcpu(vcpu, rmap_head,
								   desc, i);
					return;
				}
			}
		...
		KVM_BUG_ON_DATA_CORRUPTION(true, vcpu->kvm);
	}
}
				   ...
				   struct kvm_rmap_head *rmap_head)
{
  ...
  struct kvm_mmu_page *sp;
	...
	if (!(rmap_head->val & 1)) {
    sp = sptep_to_sp((u64 *)rmap_head->val);
    sp_write_lock(kvm, sp);
	...
	for (; desc; desc = next) {
		for (i = 0; i < desc->spte_count; i++) {
      sp = sptep_to_sp((u64 *)desc->sptes[i]);
      sp_write_lock(kvm, sp);
...
static enum kvm_rmap_type kvm_rmap_type_combine(enum kvm_rmap_type parent, enum kvm_rmap_type child)
{
  switch(parent) {
    case RMAP_EMPTY:
      if(child == RMAP_KERNEL)
        return RMAP_MIXED;
      else
        return child;
    case RMAP_MIXED:
    ...
    default:
      panic("Unexpected rmap type");
  }
}

static bool kvm_rmap_marked_kpfn_memslot(const struct kvm_memory_slot *slot, gfn_t gfn)
{
  struct kvm_rmap_head *rmap_head;
  unsigned long idx;

  idx = gfn_to_index(gfn, slot->base_gfn, PG_LEVEL_4K);
  rmap_head = &slot->arch.rmap[0][idx];
  return rmap_head->type == RMAP_KERNEL;
}

static bool kvm_mark_rmap_memslot(const struct kvm_memory_slot *slot,
    gfn_t gfn, enum kvm_rmap_type type)
{
  struct kvm_rmap_head *rmap_head;
  unsigned long idx;
...
static bool mmu_hugepage_adjust_allowed(struct kvm_vcpu *vcpu, gfn_t gfn)
{
  struct kvm_memory_slot *slot;
  struct kvm_rmap_head *rmap_head;
  unsigned long idx;
  ...
  for(i = PG_LEVEL_4K; i <= KVM_MAX_HUGEPAGE_LEVEL; i++) {
    ...
    if(rmap_head -> type != RMAP_EMPTY && rmap_head -> type != RMAP_USER)
      return false;
  }
  return true;
}
...
bool kvm_kpfn_ready_memslot(const struct kvm_memory_slot *slot, gfn_t gfn)
{
  ...
  return target->type == RMAP_KERNEL;
}
...
bool runpv_allow_hugepage_adjust(struct kvm_vcpu *vcpu, gfn_t gfn)
{
  struct kvm_memory_slot *slot;
  slot = kvm_vcpu_gfn_to_memslot(vcpu, gfn);
  return false;
...
static void rmap_remove_vcpu(struct kvm_vcpu *vcpu, u64 *spte)
{
	struct kvm_memslots *slots;
	struct kvm_memory_slot *slot;
	struct kvm_mmu_page *sp;
	 ...
	 * information in sp->role.
	 */
	slots = kvm_memslots_for_spte_role(vcpu->kvm, sp->role);

	slot = __gfn_to_memslot(slots, gfn);
	rmap_head = gfn_to_rmap(gfn, sp->role.level, slot);

	pte_list_remove_vcpu(vcpu, spte, rmap_head);
}
...
static bool rmap_write_protect(struct kvm_rmap_head *rmap_head, bool pt_protect)
{
  bool flush;
  ___kvm_rmap_read_begin(rmap_head);
  flush = __rmap_write_protect(rmap_head, pt_protect);
  ___kvm_rmap_read_end(rmap_head);
  return flush;
}
...
{
  bool flush;
	flush = __kvm_zap_rmap(kvm, rmap_head, slot);
  return flush;
}
						 ...
						 rmap_handler_t handler)
{
    ...
    ___kvm_rmap_write_begin(iterator.rmap);
		       ...
		       u64 *spte, gfn_t gfn, unsigned int access)
{
  ...
  if(!kvm_rmap_marked_kpfn_memslot(slot, gfn))
    kvm_mark_rmap_memslot(slot, gfn, RMAP_USER);
  ___kvm_rmap_write_end(rmap_head);
...
static void kvm_mmu_free_shadow_page(struct kvm *kvm, struct kvm_mmu_page *sp)
{
  ...
  unsigned long hash = kvm_page_table_hashfn(sp->gfn);

  spin_lock(&kvm->arch.mmu_page_hash_locks[sp->role.level][hash]);
  spin_lock(&kvm->arch.active_mmu_lock);

	hlist_del_rcu(&sp->hash_link);
  list_del_rcu(&sp->lru_link);
  sp_write_lock(kvm, sp);
	list_del_init(&sp->invalid_link);
  sp->role.zapped = 1;
  sp_write_unlock(kvm, sp);

  spin_unlock(&kvm->arch.active_mmu_lock);
  spin_unlock(&kvm->arch.mmu_page_hash_locks[sp->role.level][hash]);

  call_srcu(&kvm->arch.mmu_srcu, &sp->rcu_head, __kvm_mmu_free_shadow_page);
}
			    ...
			    u64 *parent_pte)
{
  ...
  if((mmu_spte_get_lockless(parent_pte) & SPTE_BASE_ADDR_MASK) == __pa(sp->spt)) {
...
#define for_each_valid_sp_lockless(_kvm, _sp, _list)				\
	hlist_for_each_entry(_sp, _list, hash_link)			\
		if (is_obsolete_sp((_kvm), (_sp))) {} else

#define for_each_valid_sp(_kvm, _sp, _list)				\
	hlist_for_each_entry(_sp, _list, hash_link)			\
		if (sp_read_lock((_kvm), (_sp))) {			\
      sp_read_unlock((_kvm), (_sp)); \
		} else

#define for_each_valid_sp_rcu(_kvm, _sp, _list) \
...
#define for_each_gfn_valid_sp_with_gptes_lockless(_kvm, _sp, _gfn, _level)		\
	for_each_valid_sp_lockless(_kvm, _sp,					\
	  &(_kvm)->arch.mmu_page_hash[(_level)][kvm_page_table_hashfn(_gfn)])	\
		if ((_sp)->gfn != (_gfn) || !sp_has_gptes(_sp)) {} else

#define for_each_valid_sp_stale(_kvm, _sp, _list)				\
	hlist_for_each_entry(_sp, _list, hash_link)			

#define for_each_gfn_valid_sp_with_gptes_stale(_kvm, _sp, _gfn, _level)		\
	for_each_valid_sp_stale(_kvm, _sp,					\
	  &(_kvm)->arch.mmu_page_hash[(_level)][kvm_page_table_hashfn(_gfn)])	\
		if ((_sp)->gfn != (_gfn) || !sp_has_gptes(_sp)) {} else
...
static bool kvm_mmu_remote_flush_or_zap(struct kvm *kvm,
					...
					bool remote_flush)
{
	if (!remote_flush && invalid_list_empty(invalid_list))
		return false;

	if (!invalid_list_empty(invalid_list))
...
static struct kvm_mmu_page *kvm_mmu_find_shadow_page_fast(struct kvm *kvm,
						     struct kvm_vcpu *vcpu,
						     gfn_t gfn,
						     struct hlist_head __rcu *sp_list,
						     union kvm_mmu_page_role role)
{
	struct kvm_mmu_page *sp;
	int collisions = 0;
...
static struct kvm_mmu_page *kvm_mmu_find_shadow_page_slow(struct kvm *kvm,
						     struct kvm_vcpu *vcpu,
                 struct shadow_page_caches *caches,
						     gfn_t gfn,
						     struct hlist_head __rcu *sp_list,
						     ...
						     union kvm_mmu_page_role role)
{
  ...
  struct kvm_mmu_invalid_list invalid_list;
	...
	for_each_valid_sp(kvm, sp, sp_list) {
		...
		my_role.access = 0;
		// pr_info("%s %d find same gfn my word %#lx his word %#lx\n", __func__, __LINE__, sp_role.word, my_role.word);
      ...
      sp_read_unlock(kvm, sp);
			...
			if (ret < 0 || sp_read_lock(kvm, sp)) {
				break;
      }

			WARN_ON_ONCE(!invalid_list_empty(&invalid_list));
			if (ret > 0)
				kvm_flush_remote_tlbs(kvm);
		}
...
kvm_mmu_find_shadow_page_atomic(struct kvm_vcpu *vcpu, gfn_t gfn, union kvm_mmu_page_role role)
{
	struct kvm_mmu_page *sp;
...
void *shadow_root_alloc(struct kvm_mmu_memory_cache *caches)
{
	...
	if (!(((unsigned long)candidate >> 13) & 1)) {
		...
		free_page((unsigned long)candidate);
	} else {
		spt = (void *)candidate;
	}
	return spt;
                  ...
                  int *rcu_idx)
{
  ...
  hash = kvm_page_table_hashfn(gfn);
	sp_list = &kvm->arch.mmu_page_hash[role.level][hash];
...
restart:
    ...
    spin_lock(&kvm->arch.mmu_page_hash_locks[role.level][hash]);
    spin_lock(&kvm->arch.active_mmu_lock);

    sp = kvm_mmu_find_shadow_page_slow(kvm, vcpu, caches, gfn, sp_list, role);

    spin_unlock(&kvm->arch.active_mmu_lock);
    spin_unlock(&kvm->arch.mmu_page_hash_locks[role.level][hash]);
             ...
             int *rcu_idx)
{
	...
	return kvm_mmu_get_shadow_page(vcpu, gfn, role, rcu_idx);
}

static struct kvm_mmu_page *kvm_mmu_get_child_sp_atomic(struct kvm_vcpu *vcpu,
						 u64 *sptep, gfn_t gfn,
						 bool direct, unsigned int access)
{
	union kvm_mmu_page_role role;
					...
					u64 addr)
{
  ...
  iterator->start_level = iterator->level;
  memset(iterator->pages, 0, sizeof(iterator->pages));
}
...
static bool shadow_walk_okay_locked(struct kvm_shadow_walk_iterator *iterator)
{
  struct kvm_mmu_page *sp;
  u64 *old_sptep = iterator->sptep;

  if(!shadow_walk_okay(iterator))
    return false;

  if(old_sptep == iterator->sptep)
    return true;

  sp = sptep_to_sp((u64 *)iterator->sptep);
  read_lock(&sp->lock);
  iterator->pages[iterator->level] = sp;
  iterator->obsolete = sp->role.invalid;
  return true;
}
...
static void shadow_walk_end(struct kvm_shadow_walk_iterator *iterator)
{
  for(int i = iterator->level; i <= iterator->start_level; i++)
    if(iterator->pages[i]) {
      read_unlock(&iterator->pages[i]->lock);
    }
}
			       ...
			       struct kvm_mmu_page *sp, bool flush)
{
    ...
    /* SPTE could already be present at the time the lock is upgraded, we need to skip it */
    if(spte_to_child_sp(*sptep) == sp)
      return;
		drop_obsolete_spte(kvm, sptep, flush);
  }
...
static int mmu_page_zap_pte(struct kvm *kvm, struct kvm_mmu_page *sp,
			    u64 *spte, struct kvm_mmu_invalid_list *invalid_list)
{
	u64 pte;
	struct kvm_mmu_page *child;
	...
	if (is_shadow_present_pte(pte)) {
      ...
      sp_write_lock(kvm, child);
			drop_parent_pte(kvm, child, spte);
      sp_write_unlock(kvm, child);
...
static void kvm_mmu_unlink_parents(struct kvm *kvm, struct kvm_mmu_page *sp)
{
  ...
  struct kvm_mmu_page *parent;
    ...
    parent = sptep_to_sp(sptep);
    sp_write_lock(kvm, parent);
		drop_parent_pte(kvm, sp, sptep);
    sp_write_unlock(kvm, parent);
  }
}
				       ...
				       int *nr_zapped)
{
  ...
  if(!list_empty(&sp->invalid_link) || sp->role.zapped) {
    *nr_zapped = 0;
    sp_write_unlock(kvm, sp);
    return 0;
  }
			    ...
			    gfn_t gfn, bool can_unsync, bool prefetch)
{
  ...
  rcu_idx = srcu_read_lock(&kvm->arch.mmu_srcu);
  for_each_level(level, 0, PT64_ROOT_MAX_LEVEL)
	for_each_gfn_valid_sp_with_gptes_stale(kvm, sp, gfn, level) {
		if (!can_unsync){
      srcu_read_unlock(&kvm->arch.mmu_srcu, rcu_idx);
			return -EPERM;
    }
...
static int mmu_set_spte_atomic(struct kvm_vcpu *vcpu, struct kvm_memory_slot *slot,
			...
			kvm_pfn_t pfn, struct kvm_page_fault *fault)
{
	...
	if (*sptep == spte) {
		...
		flush |= mmu_spte_update(sptep, spte);
		// if (fault && fault->addr >= vcpu->kvm->page_offset_base) {
		// 	pr_info("%s:%d: fault->addr %#lx, fault->error_code %#lx, sptep %#lx, spte %#lx\n", __func__, __LINE__, fault->addr, fault->error_code, sptep, spte);
		// }
		trace_kvm_mmu_set_spte(level, gfn, sptep);
	}
  ...
  runpv_debugln("sptep=0x%llx, spte=0x%llx, ret=%d", (unsigned long)sptep, spte, ret);
			      ...
			      int max_level)
{
  ...
  if(IS_ENABLED(CONFIG_KVM_RUNPV))
    ...
    read_lock(&linfo->lock);
		if (!linfo->disallow_lpage){
      read_unlock(&linfo->lock);
			break;
    }
    read_unlock(&linfo->lock);
	}
...
void kvm_mmu_hugepage_adjust(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
  ...
  if(!mmu_hugepage_adjust_allowed(vcpu, fault->gfn))
    return;
	/*
...
void *runpv_gpa_to_va(struct kvm_vcpu *vcpu, unsigned long gpa)
{
	struct kvm_memory_slot *slot;
  kvm_pfn_t pfn;
	gfn_t gfn;
 ...
 */
static struct kvm_mmu_page *runpv_mmu_get_sptep(struct kvm_vcpu *vcpu,
	            union kvm_mmu_page_role role, gpa_t ptep_pa,
	            u64 **sptep)
{
	gfn_t gfn;
	struct kvm_mmu_page *sp;
	int ret;
  gfn = ptep_pa >> PAGE_SHIFT;

retry:
	sp = kvm_mmu_find_shadow_page_atomic(vcpu, gfn, role);
	if (!sp)
 ...
 */
static int runpv_mmu_get_access(
	struct kvm_mmu_page *parent_sp,
	u64 pte
) {
	int access = parent_sp->role.access;
 ...
 */
static long runpv_mmu_write_spte(struct kvm_vcpu *vcpu,
	                    u64 *sptep, int access, u64 pte,
	                    int level)
{
	gfn_t gfn;
	struct kvm_mmu_page *sp;
 ...
 */
static long runpv_mmu_write_guest_pte(u64 *gptep,
	                    u64 new, u64 *old)
{
	if (!gptep)
		return -EFAULT;
	if (old)
		*old = READ_ONCE(*gptep);
	WRITE_ONCE(*gptep, new);
	return 0;
}

long runpv_mmu_entry_set(struct kvm_vcpu *vcpu,
	                  int level, gpa_t ptep_pa,
	                  u64 gpte)
{
	union kvm_mmu_page_role role;
	struct kvm_mmu_page *sp;
  ...
  role = vcpu->arch.mmu->root_role;
	...
	if (!(gpte & _PAGE_PRESENT) && level == PVM_SET_PTE_PTE) {
...
unsigned long runpv_mmu_entry_get(
	              struct kvm_vcpu *vcpu, int level,
	              gva_t ptep_va)
{
	gpa_t ptep_pa;
	u64 *gptep;
  gfn_t gfn;
	u64 idx, spte, gpte, gbits, sbits;
	void **gpt2spt;
	void *spt;

  ptep_pa = gva_to_gpa(vcpu, ptep_va);
  gptep = runpv_gpa_to_va(vcpu, ptep_pa);
	...
	if (spt == NULL)
		return READ_ONCE(*gptep);

	idx = (ptep_va & (PAGE_SIZE - 1)) / sizeof(*gptep);
	do {
		gpte = READ_ONCE(*gptep);
		smp_rmb();
		if (!(gpte & _PAGE_PRESENT))
			return gpte;
		spte = READ_ONCE(((unsigned long *)spt)[idx]);
		smp_rmb();
	} while (unlikely(gpte != READ_ONCE(*gptep)));

	gbits = gpte & ~(_PAGE_ACCESSED | _PAGE_DIRTY);
	sbits = spte & (_PAGE_ACCESSED | _PAGE_DIRTY);
	return gbits | sbits;
}

unsigned long runpv_mmu_ptep_get_and_clear(
	struct kvm_vcpu *vcpu, gpa_t ptep_pa,
  bool local)
{
	union kvm_mmu_page_role role;
	struct kvm_mmu_page *sp;
  ...
  gpte = READ_ONCE(*gptep);
	...
	return runpv_compose_pte(gpte, old_spte);
}

unsigned long runpv_mmu_ptep_set_wrprotect(
	struct kvm_vcpu *vcpu, gva_t ptep_pa
)
{
	union kvm_mmu_page_role role;
	struct kvm_mmu_page *sp;
  ...
  role = vcpu->arch.mmu->root_role;
	...
	WRITE_ONCE(*gptep, READ_ONCE(*gptep) & ~(unsigned long)_PAGE_RW);
...
unsigned long runpv_mmu_ptep_set_access_flags(
    struct kvm_vcpu *vcpu, gpa_t ptep_pa,
    u64 entry, bool dirty)
{
	union kvm_mmu_page_role role;
	struct kvm_mmu_page *sp;
	u64 *sptep, *gptep;
	u64 spte, gpte, old_spte, new_spte;
  u64 ad_bits;
	int rcu_idx;
	int changed;
  ...
  role = vcpu->arch.mmu->root_role;
	...
	spte = !IS_ERR(sp)? READ_ONCE(*sptep) : 0;
	changed = (runpv_compose_pte(gpte, spte) != entry);

	if (changed && dirty) {
		if (!IS_ERR(sp)) {
			ad_bits = entry & (_PAGE_ACCESSED | _PAGE_DIRTY);
			do {
				old_spte = READ_ONCE(*sptep);
...
unsigned long runpv_mmu_ptep_test_and_clear_young(
	...
	struct kvm_mmu_page *sp;
	u64 *sptep, *gptep;
  u64 gpte, spte, old_spte, new_spte;
  unsigned long ret;
	int rcu_idx;
  ...
  role = vcpu->arch.mmu->root_role;
	...
	if (runpv_compose_pte(gpte, spte) & _PAGE_ACCESSED) {
		if (!IS_ERR(sp)) {
			do {
				old_spte = READ_ONCE(*sptep);
		...
		WRITE_ONCE(*gptep, gpte & ~(unsigned long)_PAGE_ACCESSED);
		ret = 1;
	}
...
int runpv_mmu_alloc_pte(
  ...
  role = vcpu->arch.mmu->root_role;
	...
	BUG_ON(mmu_topup_memory_caches(vcpu, true));
  read_lock(&vcpu->kvm->mmu_invalidate_seq_lock);
  rcu_idx = srcu_read_lock(&vcpu->kvm->arch.mmu_srcu);
	sp = kvm_mmu_get_shadow_page(vcpu, gfn, role, &rcu_idx);
  srcu_read_unlock(&vcpu->kvm->arch.mmu_srcu, rcu_idx);
  read_unlock(&vcpu->kvm->mmu_invalidate_seq_lock);
	return 0;
}
EXPORT_SYMBOL_GPL(runpv_mmu_alloc_pte);

int runpv_mmu_release_pte(
	struct kvm_vcpu *vcpu,
	gfn_t gfn,
	int level
)
...
EXPORT_SYMBOL_GPL(runpv_mmu_release_pte);
...
static int direct_map(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
  ...
  int rcu_idx;
    ...
    rcu_idx = srcu_read_lock(&vcpu->kvm->arch.mmu_srcu);
		sp = kvm_mmu_get_child_sp(vcpu, it.sptep, base_gfn, true, ACC_ALL, &rcu_idx);
...
static int mmu_alloc_direct_roots(struct kvm_vcpu *vcpu)
{
  ...
  read_lock(&vcpu->kvm->mmu_invalidate_seq_lock);
    ...
    rcu_idx = srcu_read_lock(&vcpu->kvm->arch.mmu_srcu);
		root = mmu_alloc_root(vcpu, 0, 0, shadow_root_level, &rcu_idx);
		mmu->root.hpa = root;
    srcu_read_unlock(&vcpu->kvm->arch.mmu_srcu, rcu_idx);
...
static int mmu_alloc_shadow_roots(struct kvm_vcpu *vcpu)
{
  ...
  read_lock(&vcpu->kvm->mmu_invalidate_seq_lock);
    ...
    rcu_idx = srcu_read_lock(&vcpu->kvm->arch.mmu_srcu);
		root = mmu_alloc_root(vcpu, root_gfn, quadrant, PT32_ROOT_LEVEL, &rcu_idx);
    srcu_read_unlock(&vcpu->kvm->arch.mmu_srcu, rcu_idx);
				...
				struct kvm_page_fault *fault)
{
  ...
  bool obsolete = false;
	...
	if (sp) {
    obsolete = sp_read_lock(vcpu->kvm, sp);
    sp_read_unlock(vcpu->kvm, sp);
  }
				     ...
				     unsigned long end_gfn)
{
  ...
  struct kvm_kpfn_element *elem;
	...
	BUG_ON(start_gfn >= end_gfn);
	BUG_ON(!kvm->page_offset_base);

	for (gfn = start_gfn; gfn < end_gfn; gfn++) {
		r = mmu_topup_memory_caches(vcpu, true);
		BUG_ON(r);

		if (!(slot = kvm_vcpu_gfn_to_memslot(vcpu, gfn)))
			continue;
    ...
    elem = &slot->arch.kpfn_elems[gfn - slot->base_gfn];
		...
		sptep = NULL;
    do {
      r = paging64_init_direct_mapping_shadow(vcpu, addr, &sptep);
    } while(r);
		BUG_ON(r);

		elem->direct_map_sptep = sptep;
	}
...
EXPORT_SYMBOL_GPL(runpv_faultin_direct_mapping_fast);
...
static __always_inline bool __walk_slot_rmaps_lockless(struct kvm *kvm,
					      const struct kvm_memory_slot *slot,
					      slot_rmaps_handler fn,
					      int start_level, int end_level,
					      gfn_t start_gfn, gfn_t end_gfn,
...
static void kvm_zap_obsolete_pages(struct kvm *kvm)
{
	struct kvm_mmu_page *sp;
  struct kvm_mmu_invalid_list invalid_list;
	int nr_zapped, batch = 0, rcu_idx;
	bool unstable;
  INIT_INVALID_LIST(invalid_list);

restart:
  rcu_idx = srcu_read_lock(&kvm->arch.mmu_srcu);
	list_for_each_entry_rcu(sp, 
	      &kvm->arch.active_mmu_pages, lru_link) {
		/*
		 * No obsolete valid page exists before a newly created page
		 * since active_mmu_pages is a FIFO list.
		 */
		if (!sp_read_lock(kvm, sp)){
      sp_read_unlock(kvm, sp);
			break;
    }
...
void kvm_mmu_init_vm(struct kvm *kvm)
{
  ...
  for(int i = 0; i <= PT64_ROOT_MAX_LEVEL; i++)
    for(int j = 0; j < KVM_NUM_MMU_PAGES; j++)
      spin_lock_init(&kvm->arch.mmu_page_hash_locks[i][j]);
...
static bool kvm_rmap_mark_gfn_range(struct kvm *kvm, gfn_t gfn_start, gfn_t gfn_end, 
    enum kvm_rmap_type type)
{
	...
	for (i = 0; i < KVM_ADDRESS_SPACE_NUM; i++) {
      ...
      for (gfn = start; gfn < end; gfn++) 
          kvm_mark_rmap_memslot(memslot, gfn, type);
...
static void __kvm_mark_gfn_range(struct kvm *kvm, gfn_t gfn_start, gfn_t gfn_end,
    enum kvm_rmap_type type)
{
  bool flush;
  write_lock(&kvm->mmu_invalidate_seq_lock);
	...
	flush = kvm_rmap_mark_gfn_range(kvm, gfn_start, gfn_end, type);
...
static void __kvm_vcpu_mark_gfn_range(struct kvm_vcpu *vcpu, gfn_t gfn_start, gfn_t gfn_end, enum kvm_rmap_type type)
{
  return __kvm_mark_gfn_range(vcpu->kvm, gfn_start, gfn_end, type);
}
    ...
    int *rcu_idx)
{
	...
	return __kvm_mmu_get_shadow_page(kvm, NULL, &caches, gfn, role, rcu_idx);
}
...

{
  ...
  rcu_idx = srcu_read_lock(&kvm->arch.mmu_srcu);
	sp = shadow_mmu_get_sp_for_split(kvm, huge_sptep, &rcu_idx);
...
void runpv_mark_kpfn(struct kvm_vcpu *vcpu, gfn_t gfn, int order)
{
  ...
  runpv_debugln("gfn=0x%llx,order=%d",gfn, order);
  __kvm_vcpu_mark_gfn_range(vcpu, gfn_start, gfn_end, RMAP_KERNEL);
}
